...
## 安装 .eslintrc.js 配置文件中相关的依赖

```bash
npm install babel-eslint eslint-config-prettier eslint-plugin-prettier eslint-plugin-vue -D
```

## 在 package.json 文件中增加命令

```bash
npm set-script eslint "eslint --cache --fix"
```

执行上面那行命令会在`package.json`文件中，添加如下命令：
...
在项目中执行`npm run eslint`命令，会自动校验项目中的代码风格(有一些需要手动修复)
